...
@php $pageTitle = 'Tambah Spesialisasi'; $breadcrumb = 'Admin / Spesialisasi / Tambah'; @endphp
@section('content')
<div style="max-width:560px">
    <div class="card card-body">
        <p style="font-size:15px;font-weight:700;color:#0f172a;margin-bottom:20px">Tambah Spesialisasi</p>
        <form method="POST" action="{{ route('admin.spesialisasi.store') }}">
            @csrf
            @if($errors->any())
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_spesialis" value="{{ old('nama_spesialis') }}" class="form-input" required maxlength="100" placeholder="cth: Penyakit Dalam, Anak, Jantung...">
                @error('nama_spesialis')<p style="color:#ef4444;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
            </div>

            <div class="form-group">
                <label class="form-label">Deskripsi</label>
                <textarea name="deskripsi" rows="3" class="form-input" placeholder="Keterangan singkat...">{{ old('deskripsi') }}</textarea>
            </div>
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;gap:8px;align-items:center">
                        <span id="icon-preview" style="width:32px;height:32px;background:#dbeafe;color:#1d4ed8;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0">
                            <i class="fas {{ old('icon','fa-stethoscope') }}"></i>
                        </span>
                        <input type="text" name="icon" id="icon-input" value="{{ old('icon','fa-stethoscope') }}" class="form-input" maxlength="60" placeholder="fa-stethoscope">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select name="warna" id="warna-select" class="form-input">
                        @foreach(\App\Models\Spesialisasi::WARNA_OPTIONS as $val => $label)
                            <option value="{{ $val }}" {{ old('warna','blue') === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div style="display:flex;align-items:center;gap:10px">
                    <input type="number" name="estimasi_menit" value="{{ old('estimasi_menit',15) }}" class="form-input" min="1" max="300" style="width:120px">
